...
func (t *udp) sendPing(toid NodeID, toaddr *net.UDPAddr, callback func()) <-chan error {
	...
	log.Info("sendPing","from.ip",req.From.IP,"from.udp",req.From.UDP,"from.tcp",req.From.TCP,"toid",toid)
	log.Info("sendPing","to.ip",req.To.IP,"to.udp",req.To.UDP,"to.tcp",req.To.TCP,"toid",toid)
...
func (req *ping) handle(t *udp, from *net.UDPAddr, fromID NodeID, mac []byte) error {
	...
	t.send(from, pongPacket, &pong{
	    //返回客户端的外网ip,udp和tcp端口
		To:         makeEndpoint(from, req.From.TCP),
		ReplyTok:   mac,//返回一个哈希值 ping包中带的
		//同样发送了过期时间
		Expiration: uint64(time.Now().Add(expiration).Unix()),
	})
...
func (req *pong) handle(t *udp, from *net.UDPAddr, fromID NodeID, mac []byte) error {
	...
	log.Info("receive pong","fromID",fromID,"from",from.String(),"to",req.To.IP)
...
```
cfg.NodeDBPath  /Users/daminyang/workspace/simplechain/main/sipe/nodes
```