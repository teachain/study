...
type (
	...
	ping struct {
		...
		//注意这里是两个字段
		//一个是From,从测试的结果来看，from的ip为空，tcp和udp都带上值
		
		//一个是To，这个其实算是告诉对方，对方他自己的对外的地址和端口，
		//从测试的结果来看，ip和udp都有值，tcp为0
		
		//它们都是rpcEndpoint类型
		From, To   rpcEndpoint
		
		//过期时间,就是说接收到该数据包的节点，检查这个字段
		//如果该字段小于当前时间，所以已经过时了，可以考虑把该数据包扔掉
		//不做任何有效的处理
		Expiration uint64
