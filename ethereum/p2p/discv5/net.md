...
loop:
	for {
		...
		case q := <-net.queryReq:
			...
			//这里要非常注意它的技巧，当不能从本地获取到节点或者从网络从查询时，(此时它已经发完ping了)
			//它返回的是false,这样，当定时到的时候，就会发起邻居节点的查找
			if !q.start(net) {
    ...
    //定时完全由delay这个值决定
		case <-topicRegisterLookupTick.C:
			...
			if (topicRegisterLookupTarget.target == common.Hash{}) {
				...
				go func() { 
				    //只有这里有结果返回给topicRegisterLookupDone
				    //只有这个有值以后
				    //nodes := <-topicRegisterLookupDone这个分支才可以走
				    //主要是根据target来获取一批节点
...
func (net *Network) refresh(done chan<- struct{}) {
	...
	//如果数据库文件中没有节点数据，那么就拿种子节点来填充
	if len(seeds) == 0 {
		seeds = net.nursery
	}
	//如果数据库没有，种子节点也没有，那么就直接返回了，没法进行。
	if len(seeds) == 0 {
	...
	for _, n := range seeds {
		...
		//如果节点的状态为未知状态，那么就进行状态转换
		//从代码实现，我们知道verifyinit的enter方法是发送了一个ping消息
		if n.state == unknown {
		...
		// 将此节点先加入k桶中(内存)，后面如果这个节点不能确认在线的话，会移除的
		//  这样子当下面（Lookup）去查找邻居节点的时候，就肯定有节点数据了
		net.tab.add(n)
	}
	//发起一次自查找来填充K桶
	// Start self lookup to fill up the buckets.
	go func() {
	  //这里它不关心它的返回值，那么它的主要作用应该是刷桶
...
//当不能查找邻居节点时（如果是本节点当然是可以从本地查，当时如果remote是别的节点，那就必然是从网络中
//找，当如果此时该节点并不是可查询的状态，那么就只能给这个节点发送ping消息，然后start方法返回false
//此时先加入一个队列中，稍后再查
func (n *nodeNetGuts) deferQuery(q *findnodeQuery) {
	n.deferredQueries = append(n.deferredQueries, q)
}
//当一个节点从unknown转为know之后，会调用一次该方法
//就可以执行一次查找邻居节点的操作
//或者是它已经完成了一次查找邻居节点的操作，我们看一下延时队列是否为空，不为空，我们进行下一个查询操作
//分别是
// 1、known的enter方法
// 2、handleNeighboursPacket方法
func (n *nodeNetGuts) startNextQuery(net *Network) {
	if len(n.deferredQueries) == 0 {
		return
	}
	nextq := n.deferredQueries[0]
	//如果可以查询，那么就从队列中移除，如果还不能，继续保持
	if nextq.start(net) {
...
func init() {
	unknown = &nodeState{
		...
		handle: func(net *Network, n *Node, ev nodeEvent, pkt *ingressPacket) (*nodeState, error) {
			switch ev {
			case pingPacket:
			  //对客户端（主动ping）而已,这里它肯定是先收到pong消息，然后才收到ping消息
...
func (net *Network) handleNeighboursPacket(n *Node, pkt *ingressPacket) error {
	...
	for i, rn := range req.Nodes {
		...
		if nn.state == unknown {
		  //如果得到的邻居节点是未知状态的话，我们发起ping消息
			net.transition(nn, verifyinit)
		}
	}
	// TODO: don't ignore second packet
	n.pendingNeighbours.reply <- nodes //把节点数据返回给调用方
	n.pendingNeighbours = nil //将当前的邻居节点查询请求设置为nil
	// Now that this query is done, start the next one.
	n.startNextQuery(net) //进行邻居节点下次请求
	return nil
}
...
- 定时刷新操作refreshTimer，net.refresh(refreshDone)
- 发起主动刷新reqRefresh， net.refresh(refreshDone)
- 发现Table表中的count为0时也进行刷新操作， net.refresh(refreshDone)



这里我们总结一下，一开始K桶都是空的，要么从数据库文件中取节点，要么从提供的种子节点列表中取节点，然后把它们加入到K桶中，接着发起lookup,这个时候就从以本节点为target发起查询,此时首先从k桶中获取节点，然后向这些节点发起查找邻居节点的请求，这些他们的状态一开始是未知装状态，那么我们对它发起ping消息，如果在线，我们会最终得到一个known状态的节点，并加入到k桶中